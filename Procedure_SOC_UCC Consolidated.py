...
df_Procedures = pd.read_csv(path_wip_output + 'Combined_procedure.csv')
...
df_Procedures.loc[df_Procedures['Sub-Specialty_Final'].isin(
    ['Alex Fast Medicine', 'Alex Chronic']), 'Sub-Specialty_Final'] = 'Alex Fast Med/Chronic'
df_Procedures.loc[df_Procedures['Sub-Specialty_Final'].isin(
    ['Alex Fast General Surgery', 'Alex Chronic General Surgery']), 'Sub-Specialty_Final'] = 'Alex General Surgery'
...
# Filter to keep unique case numbers
df_Procedures_OP_Unique = df_Procedures.drop_duplicates(subset=['Case_No'])
...
procedures_df_1 = pd.pivot_table(df_Procedures_OP, values='cnt',
                                 index=['Sub-Specialty_Final', 'Treatment_OU', 'Pat_Class'],
...
procedures_df_9_1 = df_OT_Irreg[
    ['Operation_Date', 'Case_No', 'Sub-Specialty', 'Treatment_OU', 'Surgeon', 'Surgeon_MCR_No']]
...
procedures_df_10_1 = pd.pivot_table(df_Procedures, values='cnt',
                                    index=['Surgeon', 'Sub-Specialty_Final', 'Surgeon_Spec'],
                                    columns=['Year', 'Month_Number', 'Month'],
                                    aggfunc=np.sum, margins=True, margins_name='Total')
...
procedures_df_wc = (procedures_df_wc.sort_values(by='Total', ascending=True))[
    ['Surgeon', 'Surgeon_Spec']].drop_duplicates(subset=[('Surgeon', '', '')], keep='first')
...
df_Procedures_Irreg = df_Procedures_Irreg[
    ['Operation_Date', 'Case_No', 'Sub-Specialty', 'Treatment_OU', 'Surgeon', 'Surgeon_MCR_No']]
...
# read source data
df_SOC = pd.read_csv(path_wip_output + 'Combined_SOC.csv')
# dates read in csv is read as object so reconvert it into datetime object
df_SOC['Visit_Date'] = pd.to_datetime(df_SOC['Visit_Date'])
# df_SOC = pd.read_parquet(path_wip_output + 'Combined_SOC.parquet')

# Generate subset SOC df for last year statistics
df_SOC = df_SOC[df_SOC['Visit_Date'] >= first_one_year_ago]
...
psychMedMCR = ['L11767F', 'L17139E']
cardioMCR = ['L16082B', 'L18700C']
df_SOC.loc[df_SOC['Attn_MCR'].isin(psychMedMCR), 'Trt_OU'] = 'Psych Medicine (I-Care)'
...
df_SOC.loc[df_SOC['Visit_Type'].isin(['RV', 'RW', 'DR', 'RD']), 'Visit_Tagging'] = 'Returning'
...
df_SOC.loc[df_SOC['Visit_Type'].isin(['FV', 'RV', 'FW', 'RW']), 'Telehealth_Status'] = 'Non-Telehealth'
...
df_SOC5 = pd.pivot_table(df_SOC_Mapped, values='cnt',
                         index=['Resident', 'Sub-Specialty', 'Visit_Tagging', 'Pat_Class', 'Telehealth_Status'],
                         columns=['Year', 'Month_Number', 'Month'],
                         aggfunc=np.sum, margins=True, margins_name='Total')
...
df_SOC8 = pd.pivot_table(df_SOC_Mapped_Sub, values='cnt', index=['Visit_Tagging', 'Clinical_Dept', 'Sub-Specialty'],
...
# Read source data
df_UCC = pd.read_csv(path_wip_output + 'Combined_UCC.csv')
...
report.sheets[12]['A1:Z2000'].copy()
template.sheets[13]['A4'].paste()

template.sheets[0].activate()

# Save file
template.save(path_report_output + 'HIM Report (SOC,Procedures,UCC)'+reportMonth+'.xls')
template.close()
report.close()
macrobk.close()
app.quit()
